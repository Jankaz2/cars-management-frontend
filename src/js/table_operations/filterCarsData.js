    ...
    static filterCarsByBtn() {
        ...
        searchCarsBtn.addEventListener('click', (e) => {
            ...
            if (model.length === 0 && minMileage.length === 0 && maxMileage.length === 0 && color.length === 0 && priceMin.length === 0
                && priceMax.length === 0 && (componentBox1Value === 'component' && componentBox2Value === 'component2')) {
                showDataForCars(``)
            } else {
                showDataForCars(`/filter/${model.length > 0 ? model.toUpperCase() : 'default-model'}/${priceMin.length > 0 ? priceMin : '0'}/${priceMax.length > 0 ? priceMax : 'max-price'}/${color.length > 0 ? color.toUpperCase() : 'default-color'}/${minMileage.length > 0 ? minMileage : '0'}/${maxMileage.length > 0 ? maxMileage : 'max-mileage'}/${componentBox1Value !== 'component' ? componentBox1Value.toUpperCase() : componentBox1Value},${componentBox2Value !== 'component2' ? componentBox2Value.toUpperCase() : componentBox2Value}`)
            }
            e.preventDefault();
        });
    }
